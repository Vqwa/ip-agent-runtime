...
## Phase-5 / HAG-SEC-013 supply chain (2026-06-13 — hash-locked deps + SBOM + image signing)

The image build went from a bare `pip install -e . && pip install <loose list>` to a
hash-pinned, provenance-signed pipeline. The private signing key lives only in Cloud KMS
(`hosted-agents/cosign-image-signer`); verification uses the committed **public** key, so the
deploy gate needs no KMS access. Verified end-to-end: build5 SUCCESS (hash-locked install +
ci-guard-in-image + push + SBOM + cosign sign + SPDX attest all passed); `cosign verify --key
deploy/cosign.pub` confirms "signatures were verified against the specified public key".

| File | Change | Why |
|---|---|---|
| `requirements.lock` (new) | 126 packages / 2614 hashes, generated by `uv pip compile pyproject.toml requirements-runtime-extras.in --generate-hashes --python-version 3.12 -o requirements.lock`. Does NOT include the local `hermes-agent` package (installed `-e . --no-deps` after). | Every third-party wheel is hash-pinned; a compromised mirror serving a tampered artifact fails `--require-hashes` at build. |
| `requirements-runtime-extras.in` (new) | Compile input = the Dockerfile's former loose runtime extras (e2b, mcp, ddgs, oxylabs-ai-studio, fal-client==0.13.1, exa-py, firecrawl-py, parallel-web, pyjwt[crypto], uvicorn[standard], modal==1.3.4, daytona==0.155.0) + setuptools/wheel for the build backend. | Single source of truth for regenerating the lock; keeps the pinned set in sync with the image's real extras. |
| `Dockerfile.runtime` | `pip install --require-hashes -r requirements.lock` then `pip install --no-deps --no-build-isolation -e .` (was: bare `pip install -e .` + loose list). | Locked, reproducible third-party layer; the local package adds no unpinned transitive deps. |
| `cloudbuild.yaml` | build → ci-guard-in-image → push → syft SBOM → cosign sign → cosign SPDX attest; SBOM uploaded to GCS. syft+cosign are distroless (no shell) so args pass directly and sign/scan BY TAG (they resolve tag→digest); build SA holds `cloudkms.signerVerifier` on the cosign key. | Adds provenance (a KMS-signed signature) + an SPDX SBOM attestation to every image; the ci-guard step still fails the build pre-push on any runtime-invariant regression. |
| `deploy/verify_signature.sh` (new) | Deploy-time gate: `cosign verify --key deploy/cosign.pub <image>` (non-zero exit blocks rollout) + non-blocking SBOM-attestation check. Verifies against the committed **public** key — no KMS call, no quota-project pitfalls. | An unsigned or tampered image can never reach Cloud Run via the scripted deploy path. |
| `deploy/cosign.pub` (new) | The `cosign-image-signer` public key (EC P-256), exported from KMS. | Lets the deploy gate (and anyone) verify offline without KMS access. |
| `.gcloudignore` | Excludes `apps/`, assets, plans, `.plans`, acp_registry (mirrors `.dockerignore`). | Trims the Cloud Build source tarball ~20MB so the upload no longer ReadTimeouts; skills/optional-skills/scripts stay (they ARE in the image). |

**Residual (013):** the signature gate is enforced only at the scripted deploy path
(`deploy/verify_signature.sh`). Platform-level **Binary Authorization** — which makes the check
